...
export interface BiologicalVariation {
  ...
  source?: string;
  /** Date the cited source was re-checked, YYYY-MM-DD. */
  reviewedOn?: string;
  /** Upstream record timestamp when the source exposes one. */
  sourceRecordUpdatedAt?: string;
  /** EFLM publishes CVi, not the local assay's CVa. Keep the distinction explicit. */
  cvaProvenance?: 'assumed' | 'assay-specific' | 'published';
...
export function fnv1a64Hex(input: string): string {
  // Written as BigInt() calls rather than `123n` literals: the app's tsconfig
  // targets ES2017, where BigInt literals are a syntax error. The values are
  // identical — FNV-1a 64-bit prime, 64-bit mask, and offset basis.
