...
/**
 * Validate whether this user should be given a weight-change prescription at
 ...
 *
 * @example
 * // Compare a moderate and an aggressive cut.
...
export function explainRateTradeoff(
  ...
  // Pounds, not kilograms. This module stays SI everywhere it *computes*, but
  // this string is user-facing copy and the user is in the US — a lean-mass
  // figure in kg here was the one place the display boundary leaked. The
  // conversion is inlined rather than imported so the module keeps its
  // zero-dependency guarantee.
  const KG_PER_LB = 0.45359237;
  const lb = (kg: number): string => (kg / KG_PER_LB).toFixed(1);

  const body =
    `Going at ${Math.abs(requestedRatePctBwPerWeek).toFixed(2)}% per week gets you to ${targetBodyFatPct}% body fat in about ` +
    `${fast.weeksToTarget} weeks, and costs roughly ${lb(fast.leanMassLostKg ?? 0)} lb of lean mass. ` +
    `At ${recommendedRatePctBwPerWeek.toFixed(2)}% per week it takes about ${slow.weeksToTarget} weeks and costs ` +
    `roughly ${lb(slow.leanMassLostKg ?? 0)} lb. ` +
    `So you would save about ${Math.max(0, weeksSaved)} week(s) and give up about ${lb(Math.max(0, extraLeanLost))} lb of muscle. ` +
