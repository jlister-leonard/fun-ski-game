/**
 * medication-interactions.test.ts — behavioural and safety assertions for
 * `medication-interactions.ts` and `docs/kg/specs/medication-effects.json`.
 *
 * Ported verbatim from `docs/kg/specs/algorithms/verify-medications.mjs`. Kept
 * separate from the nutrition-algorithm and personalisation suites so the three
 * fail independently.
 *
...
import {
  ...
  type PrescriptionBoundaryTopic,
} from '../medication-interactions';

/* ------------------------------------------------------------------ */
/* The JSON mirror. Read from disk rather than copied: that file is the */
/* single source of truth and lives outside src/ on purpose.           */
/* ------------------------------------------------------------------ */

interface JsonCorrection {
  defensible: boolean;
  factor?: number;
  minDaysOnDrug?: number;
  invalidatedBy?: readonly string[];
}
interface JsonLabEffect {
  analyteId: string;
  confidence: string;
...
interface JsonAgent {
  id: string;
  labEffects: readonly JsonLabEffect[];
  evidenceHonesty?: string;
}
interface JsonInteraction {
  code: string;
  level: string;
  confidence: string;
}
interface JsonConfounder {
  id: string;
  engineInstruction: string;
  notThis: string;
}
interface JsonReVerifyItem {
  rank: number;
  item: string;
}
interface EffectsJson {
  agents: readonly JsonAgent[];
  interactions: readonly JsonInteraction[];
  confounders: readonly JsonConfounder[];
  reVerifyBeforeLaunch: readonly JsonReVerifyItem[];
  policy: { tier3Rule1: string };
  nonInteractions: { entries: readonly unknown[] };
  boundaries: { upperLimitStacking: string; weightPerturbations: string };
}

const p = fileURLToPath(new URL('../../../../docs/kg/specs/medication-effects.json', import.meta.url));
const effects = JSON.parse(readFileSync(p, 'utf8')) as EffectsJson;

/* ------------------------------------------------------------------ */
/* Harness. Assertions are evaluated at collection time and registered  */
/* one-per-`it`, so the reported test count is the assertion count.     */
/* ------------------------------------------------------------------ */

let currentSection = 'preamble';
function s(name: string): void {
  currentSection = name;
  console.log(`\n— ${name}`);
}
...
const boundaryTopics: readonly PrescriptionBoundaryTopic[] = [
  'dose',
  'timing',
  'stopping',
  'switching',
...
ok('an incremental add returns only the new findings', delta.every((f) => !beforeAdd.some((p2) => p2.code === f.code)));
...
const longStanding: MedicationEntry[] = [
