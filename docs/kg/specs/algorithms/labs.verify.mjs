/**
 * labs.verify.mjs — runnable assertions for labs.ts and lab-panels.json.
 *
 * Usage:
 *   cd docs/kg/specs/algorithms
 *   npx tsc labs.ts guardrails.ts --outDir build --target ES2022 --module ES2022 \
 *       --moduleResolution bundler --strict --declaration
 *   node labs.verify.mjs
 *
...
const CAT = JSON.parse(readFileSync(new URL('../lab-panels.json', import.meta.url), 'utf8'));
...
}
{
  ...
  const bad = Object.entries(A).filter(([, a]) => {
    const keys = Object.keys(a.units).map(normalizeUnitString);
...
close('cholesterol 5.2 mmol/L → 201.1 mg/dL',
  convertToCanonical(A.cholesterol_total, 5.2, 'mmol/L').value, 201.084, 1e-3);
close('glucose 5.5 mmol/L → 99.1 mg/dL',
  convertToCanonical(A.glucose_fasting, 5.5, 'mmol/L').value, 99.0858, 1e-3);
close('triglycerides 1.7 mmol/L → 150.6 mg/dL',
  convertToCanonical(A.triglycerides, 1.7, 'mmol/L').value, 150.569, 1e-3);
close('creatinine 88.4 umol/L → 1.0 mg/dL',
  convertToCanonical(A.creatinine, 88.4, 'umol/L').value, 1.0, 2e-3);
close('vitamin D 75 nmol/L → 30.0 ng/mL',
  convertToCanonical(A.vitamin_d_25oh, 75, 'nmol/L').value, 30.045, 1e-3);
close('testosterone 20 nmol/L → 576.8 ng/dL',
  convertToCanonical(A.testosterone_total, 20, 'nmol/L').value, 576.84, 1e-2);
close('ferritin ng/mL ≡ ug/L (identity)',
  convertToCanonical(A.ferritin, 18, 'ug/L').value, 18, 0);
close('TSH mIU/L ≡ uIU/mL (identity)',
  convertToCanonical(A.tsh, 2.1, 'uIU/mL').value, 2.1, 0);
close('sodium mEq/L ≡ mmol/L (identity)',
  convertToCanonical(A.sodium, 140, 'mEq/L').value, 140, 0);
close('WBC 10*9/L ≡ 10*3/uL (identity)',
  convertToCanonical(A.wbc, 6.4, '10*9/L').value, 6.4, 0);
close('albumin 40 g/L → 4.0 g/dL',
  convertToCanonical(A.albumin, 40, 'g/L').value, 4.0, 1e-9);
close('ALT 0.5 ukat/L → 30 U/L',
  convertToCanonical(A.alt, 0.5, 'ukat/L').value, 30, 1e-9);
// The mu-fold must work end to end, not just in the string normalizer.
close('serum iron with Greek mu unit converts',
  convertToCanonical(A.iron_serum, 15, 'μmol/L').value, 83.77, 1e-2);
...
close('48 mmol/mol → 6.543 % (master equation)',
  convertToCanonical(A.hba1c, 48, 'mmol/mol').value, 6.543, 1e-3);
eq('...which rounds to the published 6.5 %',
  Number(convertToCanonical(A.hba1c, 48, 'mmol/mol').value.toFixed(1)), 6.5);
close('6.5 % → 47.5 mmol/mol', convertToCanonical(A.hba1c_ifcc, 6.5, '%').value, 47.52, 5e-2);
close('39 mmol/mol → 5.72 %', convertToCanonical(A.hba1c, 39, 'mmol/mol').value, 5.72, 1e-2);
{
  ...
  const pct = convertToCanonical(A.hba1c, 48, 'mmol/mol').value;
  close('hba1c and hba1c_ifcc are exact inverses',
    convertToCanonical(A.hba1c_ifcc, pct, '%').value, 48, 1e-6);
}
{
  ...
  const actual = convertToCanonical(A.hba1c, 48, 'mmol/mol').value;
...
{
  const p = parseReferenceRangeText('3.1 - 6.2');
  ok('"3.1 - 6.2"', p?.low === 3.1 && p?.high === 6.2);
}
{
  const p = parseReferenceRangeText('13.5–17.5'); // en dash
  ok('en-dash range', p?.low === 13.5 && p?.high === 17.5);
}
...

{
  ...
  const r = resolveReferenceRange(A.glucose_fasting, { low: 65, high: 105, unit: 'mg/dL' });
  ok('lab-supplied range wins', r.provenance === 'lab_supplied' && r.low === 65 && r.high === 105);
}
{
  const r = resolveReferenceRange(A.glucose_fasting, null);
  ...
  'no_range_supplied');
{
  ...
  const mmolRange = { low: 3.9, high: 5.5, unit: 'mmol/L', text: null, provenance: 'lab_supplied' };
...
ok('provider is NOT part of the key (cross-provider dedup)',
  computeSourceKey({ ...base, sourceName: 'Sutter' }) === computeSourceKey({ ...base, sourceName: 'Quest' }));
...
const pt = (d, v, u, src) => ({ effectiveAt: d, value: v, unit: u, status: 'final', sourceName: src });

{
  ...
  ok('...and it is the clinical floor that suppressed it',
    t.suppressedReason === 'within_clinical_floor', t.suppressedReason);
...
}
{
  ...
  ok('...against the median of prior points, not the last one',
    Math.abs(t.deltaAbs - 5.1) < 1e-9, `${t.deltaAbs}`);
...
}
{
  ...
  ok('a mixed-provider series widens the threshold', b.rcv.upPct > a.rcv.upPct,
    `${a.rcv.upPct.toFixed(1)} → ${b.rcv.upPct.toFixed(1)}`);
...
const mk = (over) => parseObservation({
...

{
  ...
  const m = f.find((x) => x.code === 'LAB_CRITICAL_VALUE').message;
...
}
{
  ...
  const g = f.find((x) => x.code === 'LAB_OUTSIDE_GOAL_RANGE');
...
}
{
  ...
  const m = f.find((x) => x.code === 'LAB_OUT_OF_GENERAL_RANGE').message;
