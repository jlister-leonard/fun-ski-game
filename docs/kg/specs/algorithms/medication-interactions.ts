...
/** An agent a user takes. `prescription` items are Tier 3 rule 1 territory. */
 ...
 * situation a user is in for PSA (finasteride) and creatinine (creatine).
 */
export const LAB_EFFECTS: readonly LabEffect[] = [
  ...
  },
  {
    ...
    confidence: 'well-established',
    message:
      'Cystatin C is less dependent on muscle than creatinine, but thyroid status, steroid use, inflammation and body composition can also affect it. A combined creatinine-cystatin C estimate is generally more accurate than either marker alone; interpret it with a clinician.',
  },
];
...
/**
 * Correct a lab value for the medications a user takes, where a defensible
 * correction exists — and refuse to invent one where it does not.
 *
 * The refusal path is the important one. Three analytes in a user's likely
 ...
 * @returns `null` when nothing a user takes affects this analyte — which is
