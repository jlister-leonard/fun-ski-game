/**
 * verify.mjs — runnable assertions for the nutrition algorithm modules.
 *
 * Usage:
 *   cd docs/kg/specs/algorithms
 *   npx tsc -p tsconfig.json      # emits ./build/*.js
 *   node verify.mjs
 *
 * Or in one shot:  node --run verify   (see package.json)
 *
...
import {
  ...
  DEFAULT_KCAL_PER_KG,
} from './build/expenditure.js';
import {
  computeMacroTargets,
  weeklyCheckIn,
  dynamicMaintenanceRate,
  defaultRatePctBwPerWeek,
} from './build/macro-targets.js';
import {
  validateProfile,
  validateTargets,
  validateWeightEntry,
...
function isoDate(dayIndex, start = Date.UTC(2026, 0, 1)) {
...
/**
 * Simulate a person.
 * @param opts.days number of days
 * @param opts.startKg starting true weight
 * @param opts.tdee function(dayIndex) -> true TDEE that day
 * @param opts.intake function(dayIndex) -> logged intake that day
 * @param opts.rho true tissue energy density
 * @param opts.noiseSd scale noise SD (kg)
 * @param opts.weighInProb probability of weighing on a given day
 * @param opts.logProb probability of logging food on a given day
 * @param opts.waterShocks add transient multi-day water-weight excursions
 */
...
function joinDays(trend, logs) {
  const byDate = new Map(logs.map((l) => [l.date, l.intakeKcal]));
  return trend.map((t) => ({
    date: t.date,
    trendKg: t.trendKg,
    intakeKcal: byDate.has(t.date) ? byDate.get(t.date) : null,
  }));
}
...

{
  ...
  const creatineEvent = { startDate: isoDate(CREATINE_DAY), type: 'creatine-start' };
  ...
  function runCheckIns(trendSeries, logs, usePerturbationFields, respectVeto) {
    const days = joinDays(trendSeries, logs).map((d, i) => (usePerturbationFields
...

{
  ...
  const carb = { startDate: '2026-03-01', type: 'carb-load' };
  ...
  const creat = { startDate: '2026-03-01', type: 'creatine-start' };
...

{
  ...
  const full = Array.from({ length: 40 }, (_, i) => ({ date: isoDate(i), trendKg: 80, intakeKcal: 2400 }));
...

{
  ...
  check(
    ...
    t.perKgLeanMass.protein >= 2.2 && t.perKgLeanMass.protein <= 3.15,
...

{
  ...
  check(
    ...
    obese.perKgBodyweight.protein < 2.0 && obese.perKgLeanMass.protein >= 2.2,
...

{
  ...
  const state = { lastAdjustedDate: '2026-03-01', currentKcal: baseTargets.kcal, lastAdjustmentSign: 0 };
  ...
  const SEEDS = 10;
  let st = { lastAdjustedDate: '2026-03-01', currentKcal: baseTargets.kcal, lastAdjustmentSign: 0 };
  ...
  for (let seed = 1; seed <= SEEDS; seed++) {
    ...
    let s2 = { lastAdjustedDate: '2026-03-01', currentKcal: baseTargets.kcal, lastAdjustmentSign: 0 };
...
{
  const profile = { sex: 'female', ageYears: 30, heightCm: 165, bodyweightKg: 62, bodyFatPct: 26, goal: 'cut' };

  // Calorie floors.
  const starve = { kcal: 700, proteinG: 120, carbG: 40, fatG: 25, targetRatePctBwPerWeek: -1.2 };
  ...
  const low = { kcal: 1100, proteinG: 130, carbG: 90, fatG: 32, targetRatePctBwPerWeek: -0.9 };
  ...
  // Rate.
  const fast = { kcal: 1400, proteinG: 130, carbG: 110, fatG: 40, targetRatePctBwPerWeek: -2.0 };
  check('a 2%/wk loss rate is blocked', validateTargets(fast, { profile, bmrKcal: 1400 }).some((f) => f.code === 'RATE_LOSS_UNSAFE' && f.level === 'block'));

  // Fat floor.
  const lowFat = { kcal: 2000, proteinG: 130, carbG: 330, fatG: 25, targetRatePctBwPerWeek: -0.5 };
  check('fat below 15% of energy is blocked', validateTargets(lowFat, { profile }).some((f) => f.code === 'FAT_BELOW_HARD_FLOOR' && f.level === 'block'));

  // Protein ceiling.
  const megaProtein = { kcal: 2400, proteinG: 250, carbG: 200, fatG: 68, targetRatePctBwPerWeek: -0.5 };
  ...
  // Energy availability.
  const athlete = { sex: 'female', ageYears: 24, heightCm: 170, bodyweightKg: 58, bodyFatPct: 18, goal: 'cut' };
  const eaLow = { kcal: 1900, proteinG: 130, carbG: 200, fatG: 55, targetRatePctBwPerWeek: -0.7 };
...

{
  ...
  const projections = rates.map((r) => {
    const p = projectBodyFatOutcome(88, 21, 14, r);
    console.log(
  ...
  const asapProfile = {
    ...
    bodyFatPct: 21, goalBodyFatPct: 14, goal: 'cut',
  };
...

{
  ...
  const cold = coldStartTdee({
    sex: 'male', weightKg: 92, heightCm: 183, ageYears: 36, bodyFatPct: 24, activityLevel: 'moderate',
  });
