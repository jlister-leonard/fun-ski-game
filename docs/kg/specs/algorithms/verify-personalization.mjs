/**
 * verify-personalization.mjs — runnable assertions for the personalization
 * modules: micronutrients.ts and dietary-guardrails.ts.
 *
 * Separate from verify.mjs (which covers weight-trend / expenditure /
 * macro-targets / guardrails and is owned by another agent) so the two can fail
 * independently and neither blocks the other.
 *
 * Usage:
 *   cd docs/kg/specs/algorithms
 *   npx tsc -p tsconfig.personalization.json
 *   node verify-personalization.mjs
 *
...
import {
  ...
  OAS_COPY,
} from './build/dietary-guardrails.js';
...
const db = JSON.parse(readFileSync(join(here, '..', 'micronutrients.json'), 'utf8'));

/** The user this app is being built for. */
const PERSON = { sex: 'male', ageYears: 38, energyKcal: 2400 };

const byId = Object.fromEntries(db.nutrients.map((n) => [n.id, n]));
...
const VALID_RISK = ['none', 'low', 'moderate', 'high', 'severe'];
const VALID_CLOSE = ['equivalent', 'good', 'partial', 'poor'];
const VALID_BASIS = [
...
for (const n of db.nutrients) {
  ...
  if (n.upperLimit && !VALID_BASIS.includes(n.upperLimitBasis)) structureOk = false;
  ...
  if (!Array.isArray(n.primaryFoodSources) || n.primaryFoodSources.length < 3) foodSourcesOk = false;
  for (const s of n.primaryFoodSources ?? []) {
...
check('magnesium is the expected supplemental-only UL-below-RDA case',
  ...
  resolveUpperLimit(byId.magnesium, PERSON) < resolveReference(byId.magnesium, PERSON).value);
...
for (const [id, sex, age, want] of expect) {
...
const kAI = resolveReference(kDef, PERSON).value;
const pillsToClose = Math.ceil(kAI / kDef.supplementCap.value);
...
const saltSub = kDef.primaryFoodSources.find((s) => /salt substitute/i.test(s.food));
check('potassium lists a KCl salt substitute as a source', saltSub !== undefined);
check('one 1/4 tsp of salt substitute beats 8 supplement units', saltSub.amount > kDef.supplementCap.value * 8);
...
const b6Overlap = overlaps.find((o) => o.nutrientId === 'vitamin_b6');
...
check('overlaps are sorted with UL-exceeding first', overlaps[0].total > overlaps[0].upperLimit);
...
const nitrateAssess = allAssess.find((a) => a.nutrientId === 'dietary_nitrate');
...
const sev = { 'well-short': 0, short: 1, 'slightly-short': 2 };
...
const VAGUE = ['consider ', 'you may wish', 'might want to', 'talk to your doctor about whether'];
...
const d3Rec = GAP_CLOSING_PROTOCOL.find((r) => r.id === 'vitamin_d');
...
check('potassium recommendation is the salt substitute, not a pill',
  /salt substitute/i.test(GAP_CLOSING_PROTOCOL.find((r) => r.id === 'potassium_salt').compound));

/* --- Honesty checks on specific recommendations ---------------------- */
const nitrateRec = GAP_CLOSING_PROTOCOL.find((r) => r.id === 'nitrate');
...
const k1Rec = GAP_CLOSING_PROTOCOL.find((r) => r.id === 'k1');
...
const magRec = GAP_CLOSING_PROTOCOL.find((r) => r.id === 'magnesium');
...
const fibreRec = GAP_CLOSING_PROTOCOL.find((r) => r.id === 'psyllium');
...
const kRec = GAP_CLOSING_PROTOCOL.find((r) => r.id === 'potassium_salt');
check('potassium caveat names CKD and the RAAS drugs',
  /kidney/i.test(kRec.caveat) && /ACE inhibitor/i.test(kRec.caveat) && /spironolactone|potassium-sparing/i.test(kRec.caveat));
check('potassium caveat flags the iodine side effect of swapping table salt',
  /iodis|iodiz/i.test(kRec.caveat));

const creatineRec = GAP_CLOSING_PROTOCOL.find((r) => r.id === 'creatine');
...
const onTarget = detectSustainedUnderEating(days(Array(14).fill(2400)));
...
const gappy = detectSustainedUnderEating(
  days(Array(14).fill(2400)).map((d, i) => (i % 2 === 0 ? { ...d, kcal: null } : d)),
);
...
check('fat floor message explains the fat-soluble vitamin consequence',
  lowFat.find((f) => f.code === 'FAT_BELOW_FLOOR').message.includes('fat-soluble'));
...
check('fiber finding is info, framed as hunger not virtue',
  lowFiber.find((f) => f.code === 'FIBER_BELOW_FLOOR').level === 'info' &&
  /hunger/.test(lowFiber.find((f) => f.code === 'FIBER_BELOW_FLOOR').message));
...
check('the recommended response is slowing the rate, not eating more foods',
  /slowing|slower|% a week/.test(implausible.find((f) => f.code === 'RESTRICTED_DIET_RATE_IMPLAUSIBLE').message));
...
check('group-coverage copy is framed as information, not instruction',
  ...
  }).find((f) => f.code === 'FOOD_GROUP_COVERAGE_THIN')?.message.includes('rather than telling you what to eat'));
...
check('OAS copy explicitly warns against self-directed challenges',
  /do not use this app.*as a reason to try a food/s.test(oasCopyAll));
...
const badBuild = validateDietary({
  trackingConfig: { ...REQUIRED_TRACKING_SAFETY, streaks: true, celebrateUnderBudget: true },
});
